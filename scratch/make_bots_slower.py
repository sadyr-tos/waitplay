...
old_timer = """    if (this.state.isDemoTest) {
...
new_timer = """    if (this.state.isDemoTest) {
      clearTimeout(this.state.demoTimer);
...
old_bots = """  simulateBotsAnswering() {
    clearInterval(this.state.gameRunningInterval);
...
new_bots = """  simulateBotsAnswering() {
    clearInterval(this.state.gameRunningInterval);
...
old_gps_toast = 'this.showVisitorToast("🛠️ Запущен демо-режим администратора вне заведения (10 сек на игру).", false);'
new_gps_toast = 'this.showVisitorToast("🛠️ Кабинет администратора: запущен тестовый режим без ограничений.", false);'
code = code.replace(old_gps_toast, new_gps_toast)
